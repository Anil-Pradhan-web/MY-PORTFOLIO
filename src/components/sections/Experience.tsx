...
export default function Experience() {
  return (
    <section id="experience" className="py-24 w-full max-w-7xl mx-auto px-6 sm:px-12 md:px-24">
      ...
      <div className="mt-12 ml-4 md:ml-8 border-l border-violet-400/50">
        {experiences.map((exp, idx) => (
          ...
          >
            {/* Timeline Dot */}
            <span className="absolute -left-2 top-2 h-4 w-4 rounded-full bg-violet-400 shadow-[0_0_10px_rgba(139,92,246,0.7)] border-4 border-[#0a0a0a]" />

            <div className="flex flex-col md:flex-row md:items-baseline md:justify-between mb-2">
              <h3 className="text-2xl font-bold font-syne text-white">{exp.role}</h3>
              <span className="text-violet-400 font-mono text-sm mt-1 md:mt-0 font-medium whitespace-nowrap">
                {exp.period}
              </span>
            </div>
