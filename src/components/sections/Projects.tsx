...
const projectsList = [
{
  ...
  tagline: "4 Specialized AI Agents. One Career Transformation. Under 60 Seconds.",
  description:
    "A production-grade, full-stack career coaching platform powered by a 4-agent Microsoft AutoGen v0.2 system with a dedicated Direct Groq WebSocket mock interviewer. Built on a decoupled microservices architecture — Next.js 14 frontend on Vercel, FastAPI backend on Render, Neon Serverless Postgres, and Upstash Redis. Features real-time WebSocket mock interviews with sub-2s Groq streaming latency, live market intelligence via Tavily AI & Serper.dev, SHA-256 keyed AI response caching, a deterministic ATS scoring engine, and Google OAuth 2.0 with JWT refresh tokens. Hybrid Dual-LLM: Gemini 2.5 Flash as primary reasoning engine with automatic programmatic failover to Groq (Llama 3.3 70B) on 429 rate limit errors — 100% system availability. 110+ commits across 5–6 months of solo development.",
  highlight:
    "4-Agent AutoGen v0.2 Orchestration · Direct Groq WebSocket Streaming · Gemini 2.5 Flash → Groq Auto-Failover · Tavily AI & Serper.dev Market Intelligence · Google OAuth 2.0 · Production CI/CD",
  stats: [
    { label: "AI Agents", value: "4+1" },
  ...
  features: [
    "📄 Resume Analyzer: PDF parsed via pdfplumber, deterministic ATS scoring engine, AI skill-gap detection with structured JSON persisted to Neon Serverless Postgres — no S3 needed",
    "🗺️ 8-Week Roadmap Generator: Career Coach agent builds personalized weekly plans enriched with real resource URLs via a custom search enrichment engine using Tavily AI & Serper.dev tool calling",
    "📈 Live Market Intelligence: Market Researcher agent fetches real-time salary benchmarks, hiring volumes, and in-demand skills via Tavily AI & Serper.dev with regex-based JSON extraction — zero stale mock data",
    "🎤 Streaming Mock Interviews: Bypasses AutoGen entirely — Direct Groq OpenAI SDK over WebSocket for sub-2s token streaming, 7-question adaptive state machine (FAANG=Hard / Service-based=Easy), Edge-TTS voice synthesis with 30s timeout guard, crash-resilient _safe_send_json() WebSocket handling",
    "🔗 LinkedIn Profile Reviewer: AutoGen agent with Gemini 2.5 Flash → Groq/Llama-3 automatic failover — headline optimization, profile SEO scoring, and recruiter keyword gap analysis",
    "🧠 Full Multi-Agent Analysis: GroupChatManager with custom speaker selection orchestrates 4 AutoGen v0.2 agents in parallel via asyncio.to_thread() — complex career diagnostics consolidated in under 60 seconds",
    ...
    "🔀 Hybrid Dual-LLM Architecture: Gemini 2.5 Flash as primary reasoning engine with programmatic failover to Groq (Llama 3.3 70B) on 429 rate limit errors — 100% system availability, zero user-facing failures",
    ...
    "📊 Persistent Dashboard: Real-time Skill Radar (Recharts), Day Streaks, Weekly Activity — all backed by Neon Serverless Postgres with optimized connection pooling (pool_size=3, pool_recycle=300s, pool_pre_ping=True)",
  ...
  architecture: {
    ...
    agents: [
      {
        name: "Resume Analyst",
        engine: "AutoGen v0.2 + Gemini 2.5 Flash / Groq fallback",
        output: "ATS score, skill gaps, strengths, structured JSON to Postgres",
      },
      {
        name: "Market Researcher",
        engine: "AutoGen v0.2 + Gemini 2.5 Flash / Groq fallback + Tavily AI & Serper.dev",
        output: "Live salary benchmarks, top hiring companies, in-demand skills",
      },
      {
        name: "Career Coach",
        engine: "AutoGen v0.2 + Gemini 2.5 Flash / Groq fallback + Tavily AI & Serper.dev",
        output: "8-week roadmap with real resource URLs via search enrichment engine",
      },
      {
      ...
      },
      {
        ...
        engine: "Direct Groq OpenAI SDK Streaming (bypasses AutoGen)",
        output: "7 adaptive questions → Edge-TTS voice → score /100",
      },
    ],
    llm: {
      primary: "Google Gemini 2.5 Flash — all 4 AutoGen analysis agents",
      streaming: "Groq Llama 3.3 70B — WebSocket mock interviews + automatic 429 failover for all agents",
      failover: "Programmatic Gemini → Groq switch on 429 rate limit errors — 100% system availability",
    },
  },
  stack: [
    "Next.js 14",
  ...
  hackathons: [
    {
      ...
      prizePool: "$80,000+",
      requirements: ["Microsoft AutoGen v0.2", "Deployed MVP", "Public Repo"],
    },
    {
      name: "Amazon Nova AI",
      prizePool: "$40,000 cash + $55,000 AWS Credits",
      requirements: ["Multi-Agent System", "Edge-TTS Voice", "Full-stack Deployed MVP"],
    },
  ],
