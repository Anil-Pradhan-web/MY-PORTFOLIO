...
const projectsList = [
{
  ...
  description:
    "A production-grade, full-stack career coaching platform powered by a LangGraph static DAG orchestrating 5 specialized AI agents with a dedicated Direct Groq/NVIDIA WebSocket mock interviewer. Built on a decoupled microservices architecture — Next.js 14 frontend on Vercel, FastAPI backend on Render, Neon Serverless Postgres, and Upstash Redis. Features a Hybrid Semantic RAG Engine (ChromaDB + in-memory MockRAG OOM fallback), real-time WebSocket mock interviews with sub-2s Groq/NVIDIA streaming latency, live market intelligence via Serper.dev & Tavily AI with async link validation and resource scoring, SHA-256 keyed AI response caching (7-day TTL), a deterministic ATS scoring engine, and Google OAuth 2.0 with JWT refresh token rotation. Dynamic Multi-Provider LLM routing across Groq (Llama 3.3 70B), NVIDIA NIM (Llama 3.3 70B-Instruct), and Gemini 2.5 Flash with automatic fallback chains and circuit-breaker mechanisms — 100% system availability. 254+ commits, 83 passing tests, zero circular dependencies across 5–6 months of solo development.",
  highlight:
    "LangGraph Static DAG · ChromaDB Hybrid RAG · Groq + NVIDIA NIM + Gemini Multi-Provider · Sub-2s WebSocket Streaming · Serper.dev & Tavily Market Intelligence · Google OAuth 2.0 · Docker + GHCR + GitHub Actions CI/CD",
  stats: [
    { label: "AI Agents", value: "5" },
    { label: "Interview Latency", value: "<2s" },
    { label: "Analysis Time", value: "<60s" },
    { label: "Commits", value: "254+" },
    { label: "Pytest Tests", value: "83" },
  ...
  features: [
    ...
    "🗺️ 8-Week Roadmap Generator: LangGraph Roadmap Builder aggregates skill gaps + market trends + LinkedIn data, enriched via ChromaDB RAG (auto-seeded at startup with 50+ curated resources) + in-memory MockRAG fallback preventing OOM crashes on Render's 512MB RAM containers — real YouTube, GitHub, and documentation resources per week via multi-source deduplication pipeline",
    "📈 Live Market Intelligence: Market Researcher workflow fetches real-time salary benchmarks, hiring volumes, and in-demand skills via Serper.dev & Tavily AI with regex-based JSON extraction, async link validation (10 concurrent workers), and resource scoring engine (domain authority weights + freshness signals) — zero stale mock data",
    "🎤 Streaming Mock Interviews: Bypasses LangGraph entirely — Direct Groq/NVIDIA OpenAI SDK over WebSocket for sub-2s token streaming, 7-question adaptive state machine with 3 difficulty tiers (FAANG=Hard, Product=Medium, Service=Easy), Edge-TTS voice synthesis with 30s timeout guard, crash-resilient WebSocket cleanup preventing memory leaks across concurrent sessions",
    ...
    "🔐 Google OAuth 2.0: One-click login via @react-oauth/google — ID Token verified server-side with google-auth, exchanged for short-lived JWT + long-lived refresh token with rotation preventing replay attacks, bcrypt-secured fallback auth",
    "🔀 Dynamic Multi-Provider LLM Routing: Groq (Llama 3.3 70B) · NVIDIA NIM (Llama 3.3 70B-Instruct) · Gemini 2.5 Flash — user-selectable from settings, automatic fallback chains (groq→google, nvidia→google), deterministic fallback engines for malformed outputs, circuit-breaker on repeated failures",
    "🛡️ Rate Limiting + Caching: SlowAPI + Upstash Redis — 100 req/hr, 1000 req/day per user. SHA-256 keyed AI response cache with 7-day TTL bypasses redundant LLM calls. Rate counters increment only on successful AI responses",
    "📊 Persistent Dashboard: Real-time Skill Radar (Recharts), Day Streaks, Weekly Activity, interview history, roadmap history — all backed by Neon Serverless Postgres with connection pooling (pool_size=3, pool_pre_ping for Neon compatibility)",
    "🏗️ CI/CD Pipeline: GitHub Actions — frontend ESLint + Next.js production build, backend 83 pytest tests + pip-audit security scanning. Auto-deploy via Vercel (frontend), Render webhook (backend), Docker image publishing to GHCR. Zero circular dependencies across entire codebase",
  ],
  architecture: {
  ...
  quickStats: {
    ...
    agents: "5 (Resume, Market, LinkedIn, Roadmap, Interview)",
    llmProviders: "3 (Groq, NVIDIA NIM, Gemini)",
    responseTime: "<60s full career analysis",
    buildTime: "5–6 months solo",
    hackathonPrizePool: "$175,000+",
    deployments: "Vercel + Render + Neon + Upstash",
    codeQuality: "Zero circular deps · Pydantic validation · 83 tests · 100% availability",
  },
